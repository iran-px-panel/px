...
async function checkAutoResets(env) {
  ...
  try {
    ...
    await env.DB.prepare(
      `UPDATE users SET used_gb = 0, is_active = 1, last_reset_vol_time = ? WHERE auto_reset_vol_days > 0 AND ? >= (last_reset_vol_time + (auto_reset_vol_days * 86400000))`,
    )
      .bind(todayUtc, todayUtc)
      .run();
    await env.DB.prepare(
      `UPDATE users SET used_req = 0, is_active = 1, last_reset_req_time = ? WHERE auto_reset_req_days > 0 AND ? >= (last_reset_req_time + (auto_reset_req_days * 86400000))`,
    )
      .bind(todayUtc, todayUtc)
      .run();
  } catch (e) {}
...
async function checkAutoRotates(env) {
  ...
  try {
    const { results: usersToRotate } = await env.DB.prepare(
      "SELECT * FROM users WHERE auto_rotate_ip = 1 AND ? >= (last_rotate_time + (rotate_time * 60000))",
    )
      .bind(now)
      .all();
    ...
    for (const u of usersToRotate) {
      ...
      if (u.ip_operator === "all") {
        Object.values(cachedIpsData).forEach(
          (ips) => (availableIps = availableIps.concat(ips)),
        );
      } else {
      ...
      if (selectedIps.length > 0) {
        await env.DB.prepare(
          "UPDATE users SET ips = ?, last_rotate_time = ? WHERE id = ?",
        )
          .bind(selectedIps.join("\n"), now, u.id)
          .run();
      }
    }
  } catch (e) {}
...
async function replaceBrokenProxy(username, env, oldProxy) {
  try {
    ...
    const user = await env.DB.prepare(
      "SELECT id, user_socks5, auto_rotate_user_proxy FROM users WHERE username = ?",
    )
      .bind(username)
      .first();
    if (
      !user ||
      user.auto_rotate_user_proxy !== 1 ||
      user.user_socks5 !== oldProxy
    ) {
    ...
    let countryCode = "all";
    try {
      let remain = oldProxy.replace(
        /^(socks4|socks5|socks|http|https):\/\//i,
        "",
      );
      if (remain.includes("@"))
        remain = remain.substring(remain.lastIndexOf("@") + 1);
      if (remain.startsWith("["))
        remain = remain.substring(1, remain.indexOf("]"));
      else if (remain.includes(":"))
        remain = remain.substring(0, remain.lastIndexOf(":"));
      const geoRes = await fetch(
        `http://ip-api.com/json/${remain}?fields=countryCode`,
      );
    ...
    if (
      cachedVipCountries.length === 0 ||
      Date.now() - lastVipCountriesFetch > 3600000
    ) {
      try {
        const ghRes = await fetch(
          "https://api.github.com/repos/IR-NETLIFY/zeus/contents/proxy/proxy_vip",
          {
    ...
    if (upperCountry !== "ALL" && upperCountry !== "UN") {
      sources.push({
        url: `https://raw.githubusercontent.com/IR-NETLIFY/zeus/refs/heads/main/proxy/proxy_vip/${upperCountry}.txt`,
        type: "repo",
      });
    }
    for (const fc of fallbackVIPs) {
      if (fc !== upperCountry) {
        sources.push({
          url: `https://raw.githubusercontent.com/IR-NETLIFY/zeus/refs/heads/main/proxy/proxy_vip/${fc}.txt`,
          type: "repo",
        });
      }
    }
    if (!isOldProxyVIP) {
      if (upperCountry !== "ALL" && upperCountry !== "UN") {
        sources.push({
          url: `https://raw.githubusercontent.com/IR-NETLIFY/zeus/refs/heads/main/proxy/${upperCountry}.txt`,
          type: "repo",
        });
        sources.push({
          url: `https://api.proxyscrape.com/v3/free-proxy-list/get?request=displayproxies&protocol=socks5&country=${countryCode}&format=text`,
          type: "socks5",
        });
      }
      sources.push({
        url: `https://api.proxyscrape.com/v3/free-proxy-list/get?request=displayproxies&protocol=socks5&country=all&format=text`,
        type: "socks5",
      });
    }
      ...
      try {
        ...
        if (lines.length > 0) {
          ...
          const testBatch = lines.slice(0, 3).map((line) => {
            if (
              src.type === "repo" &&
              !line.match(/^(socks4|socks5|socks|http|https):\/\//i)
            ) {
              return `socks5://${line}`;
    ...
    if (newProxy) {
      await env.DB.prepare("UPDATE users SET user_socks5 = ? WHERE id = ?")
        .bind(newProxy, user.id)
        .run();
    }
  } catch (e) {
  } finally {
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (Router.isWebSocketUpgrade(request) && url.pathname === "/PX_PANEL") {
...
const Router = {
  ...
  async handleWebSocket(request, env, ctx) {
    try {
      ...
      try {
        ...
        const socksRow = await env.DB.prepare(
          "SELECT value FROM settings WHERE key = 'socks5'",
        ).first();
  ...
  async handleSubscription(url, env) {
    ...
    try {
      ...
      try {
        await env.DB.prepare(
          "UPDATE users SET used_req = used_req + 1 WHERE username = ?",
        )
          .bind(user.username)
          .run();
      } catch (e) {}
  ...
  async handleApi(request, url, env, ctx) {
    ...
    if (url.pathname === "/api/setup-password" && request.method === "POST") {
      if (hasPassword) {
        return new Response(
          JSON.stringify({ error: "رمز عبور از قبل تعریف شده است" }),
          {
            status: 400,
      ...
      if (!password || password.length < 4) {
        return new Response(
          JSON.stringify({ error: "رمز عبور باید حداقل ۴ کاراکتر باشد" }),
          {
            status: 400,
            headers: { "Content-Type": "application/json; charset=utf-8" },
          },
    ...
    if (url.pathname === "/api/recover" && request.method === "POST") {
      ...
      try {
        ...
        if (!cfRes.ok || !cfData.success) {
          return new Response(
            JSON.stringify({ error: "Invalid or expired Cloudflare token" }),
            {
              status: 401,
              headers: { "Content-Type": "application/json; charset=utf-8" },
            },
          ...
          if (accountsData.success && accountsData.result) {
            for (const acc of accountsData.result) {
              const subRes = await fetch(
                `https://api.cloudflare.com/client/v4/accounts/${acc.id}/workers/subdomain`,
                {
                  headers: { Authorization: "Bearer " + api_token },
                },
              );
              ...
              if (
                subData.success &&
                subData.result &&
                subData.result.subdomain === targetSubdomain
              ) {
                isAuthorized = true;
                break;
              }
        ...
        if (!isAuthorized) {
          return new Response(
            JSON.stringify({
              error: "این توکن متعلق به صاحب پنل نیست (ای کــثـــکـــش)",
            }),
            {
              status: 403,
              headers: { "Content-Type": "application/json; charset=utf-8" },
        ...
        await env.DB.prepare(
          "DELETE FROM settings WHERE key = 'panel_password'",
        ).run();
        ...
        return new Response(
          JSON.stringify({ error: "Cloudflare API connection error" }),
          {
            status: 500,
            headers: { "Content-Type": "application/json; charset=utf-8" },
          },
    ...
    if (
      (url.pathname === "/api/update-panel" ||
        url.pathname === "/api/restart-core") &&
      request.method === "POST"
    ) {
      ...
      try {
        ...
        const githubRes = await fetch(
          "https://api.github.com/repos/iran-px-panel/px/contents/px.js",
          {
            headers: {
        ...
        const bindingsRes = await fetch(
          `https://api.cloudflare.com/client/v4/accounts/${currentAccountId}/workers/scripts/${scriptName}/bindings`,
          {
            headers: { Authorization: "Bearer " + currentToken },
          },
        );
        ...
        if (!bindingsData.success)
          throw new Error(
            isUpdate
              ? "عدم دسترسی به تنظیمات ورکر. توکن نامعتبر است."
              : "عدم دسترسی به تنظیمات ورکر",
          );
        const newBindings = [];
        ...
        const deployRes = await fetch(
          `https://api.cloudflare.com/client/v4/accounts/${currentAccountId}/workers/scripts/${scriptName}`,
          {
            method: "PUT",
            headers: { Authorization: "Bearer " + currentToken },
            body: formData,
        ...
        if (!deployData.success)
          throw new Error(
            isUpdate
              ? "خطا در اعمال آپدیت در کلودفلر."
              : "خطا در اعمال ری‌استارت در کلودفلر",
          );
        return new Response(JSON.stringify({ success: true }), {
          headers: { "Content-Type": "application/json" },
        });
        ...
        const errorMsg =
          err.message +
          (isUpdate
            ? " | در صورت عدم موفقیت، از طریق لینک زیر آپدیت کنید: https://zeus-panel.ir-netlify.workers.dev/"
            : "");
    ...
    if (url.pathname === "/api/change-password" && request.method === "POST") {
      ...
      if (!current_password || !new_password) {
        return new Response(
          JSON.stringify({ error: "رمز عبور فعلی و جدید الزامی هستند" }),
          {
            status: 400,
            headers: { "Content-Type": "application/json; charset=utf-8" },
          },
      ...
      if (new_password.length < 4) {
        return new Response(
          JSON.stringify({ error: "رمز عبور جدید باید حداقل ۴ کاراکتر باشد" }),
          {
            status: 400,
            headers: { "Content-Type": "application/json; charset=utf-8" },
          },
    ...
    if (url.pathname === "/api/proxy-ip") {
      if (request.method === "POST") {
        ...
        if (socks5 !== undefined)
          await env.DB.prepare(
            "INSERT OR REPLACE INTO settings (key, value) VALUES ('socks5', ?)",
          )
            .bind(socks5)
            .run();
        return new Response(JSON.stringify({ success: true }), {
      ...
      if (request.method === "GET") {
        ...
        const rowSocks = await env.DB.prepare(
          "SELECT value FROM settings WHERE key = 'socks5'",
        ).first();
        return new Response(
          JSON.stringify({
      ...
      if (isUserAction) {
        ...
        if (request.method === "PUT") {
          ...
          if (body.toggle_only !== undefined) {
            ...
            if (body.reset_action === "volume") {
              await env.DB.prepare(
                "UPDATE users SET used_gb = 0, is_active = 1 WHERE username = ?",
              )
                .bind(username)
                .run();
              ...
              await env.DB.prepare(
                "UPDATE users SET used_req = 0, is_active = 1 WHERE username = ?",
              )
                .bind(username)
                .run();
              ...
              await env.DB.prepare(
                "UPDATE users SET created_at = CURRENT_TIMESTAMP, is_active = 1 WHERE username = ?",
              )
                .bind(username)
                .run();
            }
            ...
            if (new_username && new_username !== username) {
              if (!/^[a-zA-Z0-9_-]+$/.test(new_username)) {
                return new Response(
                  JSON.stringify({ error: "نام کاربری جدید غیرمجاز است" }),
                  {
                    status: 400,
                    headers: {
                      "Content-Type": "application/json; charset=utf-8",
              ...
              if (existing) {
                return new Response(
                  JSON.stringify({ error: "این نام کاربری از قبل وجود دارد" }),
                  {
                    status: 400,
                    headers: { "Content-Type": "application/json" },
                  },
              ...
              if (GLOBAL_TRAFFIC_CACHE.has(username)) {
                GLOBAL_TRAFFIC_CACHE.set(
                  new_username,
                  GLOBAL_TRAFFIC_CACHE.get(username),
                );
              ...
              if (ACTIVE_CONNECTIONS_COUNT.has(username)) {
                ACTIVE_CONNECTIONS_COUNT.set(
                  new_username,
                  ACTIVE_CONNECTIONS_COUNT.get(username),
                );
                ACTIVE_CONNECTIONS_COUNT.delete(username);
              }
              if (GLOBAL_LAST_ACTIVE_WRITE.has(username)) {
                GLOBAL_LAST_ACTIVE_WRITE.set(
                  new_username,
                  GLOBAL_LAST_ACTIVE_WRITE.get(username),
                );
                GLOBAL_LAST_ACTIVE_WRITE.delete(username);
              }
            }
            await env.DB.prepare(
              "UPDATE users SET username = ?, limit_gb = ?, expiry_days = ?, limit_req = ?, ips = ?, tls = ?, port = ?, fingerprint = ?, max_connections = ?, ip_limit = ?, block_porn = ?, block_ads = ?, frag_len = ?, frag_int = ?, user_proxy_iata = ?, user_socks5 = ?, user_proxy_ip = ?, auto_reset_vol_days = ?, auto_reset_req_days = ?, auto_rotate_ip = ?, rotate_time = ?, ip_operator = ?, ip_count = ?, auto_rotate_user_proxy = ? WHERE username = ?",
            )
              .bind(
                ...
                user_socks5 || null,
                user_proxy_ip || null,
                auto_reset_vol_days ? parseInt(auto_reset_vol_days) : 0,
                auto_reset_req_days ? parseInt(auto_reset_req_days) : 0,
                auto_rotate_ip || 0,
                rotate_time || 0,
                ip_operator || "all",
                ip_count || 20,
                auto_rotate_user_proxy ? 1 : 0,
                username,
              )
              .run();
            return new Response(JSON.stringify({ success: true }), {
        ...
        if (request.method === "GET") {
          ...
          const enrichedUsers = (results || []).map((user) => ({
            ...user,
            is_online:
              user.last_active && now - user.last_active < 20000 ? 1 : 0,
            online_count: getActiveIpCount(user.active_ips),
          }));
        ...
        if (request.method === "POST") {
          ...
          if (!username) {
            return new Response(
              JSON.stringify({ error: "نام کاربری اجباری است" }),
          ...
          if (username.length > 32) {
            return new Response(
              JSON.stringify({
                error: "نام کاربری نمی‌تواند بیشتر از ۳۲ کاراکتر باشد",
              }),
              { status: 400, headers: { "Content-Type": "application/json" } },
            );
          }
          if (!/^[a-zA-Z0-9_-]+$/.test(username)) {
            return new Response(
              JSON.stringify({
                error:
                  "نام کاربری غیرمجاز است (فقط حروف، اعداد، خط تیره و آندرلاین)",
              }),
              {
                status: 400,
                headers: { "Content-Type": "application/json; charset=utf-8" },
...
const DbService = {
  async ensureSchema(db) {
    ...
    try {
      await db
        .prepare("ALTER TABLE users ADD COLUMN used_req INTEGER DEFAULT 0")
        .run();
    } catch (e) {}
    try {
      await db
        .prepare("ALTER TABLE users ADD COLUMN ip_limit INTEGER DEFAULT NULL")
        .run();
    } catch (e) {}
    try {
      await db
        .prepare("ALTER TABLE users ADD COLUMN active_ips TEXT DEFAULT NULL")
        .run();
    } catch (e) {}
    try {
      await db
        .prepare(
          "UPDATE users SET ip_limit = max_connections WHERE ip_limit IS NULL AND max_connections IS NOT NULL",
        )
        .run();
    } catch (e) {}
    try {
    ...
    try {
      await db
        .prepare("ALTER TABLE users ADD COLUMN block_porn INTEGER DEFAULT 0")
        .run();
    } catch (e) {}
    try {
      await db
        .prepare("ALTER TABLE users ADD COLUMN block_ads INTEGER DEFAULT 0")
        .run();
    } catch (e) {}
    try {
      await db
        .prepare(
          "ALTER TABLE users ADD COLUMN frag_len TEXT DEFAULT '200-3000'",
        )
        .run();
    } catch (e) {}
    try {
      await db
        .prepare("ALTER TABLE users ADD COLUMN frag_int TEXT DEFAULT '1-2'")
        .run();
    } catch (e) {}
    try {
      await db
        .prepare("ALTER TABLE users ADD COLUMN lifetime_used_gb REAL DEFAULT 0")
        .run();
    } catch (e) {}
    try {
      await db
        .prepare(
          "UPDATE users SET lifetime_used_gb = used_gb WHERE lifetime_used_gb = 0 OR lifetime_used_gb IS NULL",
        )
        .run();
    } catch (e) {}
    try {
      await db
        .prepare("ALTER TABLE users ADD COLUMN user_proxy_ip TEXT DEFAULT NULL")
        .run();
    } catch (e) {}
    try {
      await db
        .prepare(
          "ALTER TABLE users ADD COLUMN user_proxy_iata TEXT DEFAULT NULL",
        )
        .run();
    } catch (e) {}
    try {
      await db
        .prepare("ALTER TABLE users ADD COLUMN user_socks5 TEXT DEFAULT NULL")
        .run();
    } catch (e) {}
    try {
      await db
        .prepare(
          "ALTER TABLE users ADD COLUMN auto_reset_vol_days INTEGER DEFAULT 0",
        )
        .run();
    } catch (e) {}
    try {
      await db
        .prepare(
          "ALTER TABLE users ADD COLUMN auto_reset_req_days INTEGER DEFAULT 0",
        )
        .run();
    } catch (e) {}
    try {
      await db
        .prepare(
          "ALTER TABLE users ADD COLUMN last_reset_vol_time INTEGER DEFAULT 0",
        )
        .run();
    } catch (e) {}
    try {
      await db
        .prepare(
          "ALTER TABLE users ADD COLUMN last_reset_req_time INTEGER DEFAULT 0",
        )
        .run();
    } catch (e) {}
    try {
      await db
        .prepare(
          "ALTER TABLE users ADD COLUMN auto_rotate_ip INTEGER DEFAULT 0",
        )
        .run();
    } catch (e) {}
    try {
      await db
        .prepare("ALTER TABLE users ADD COLUMN rotate_time INTEGER DEFAULT 0")
        .run();
    } catch (e) {}
    try {
      await db
        .prepare("ALTER TABLE users ADD COLUMN ip_operator TEXT DEFAULT 'all'")
        .run();
    } catch (e) {}
    try {
      await db
        .prepare("ALTER TABLE users ADD COLUMN ip_count INTEGER DEFAULT 20")
        .run();
    } catch (e) {}
    try {
      await db
        .prepare(
          "ALTER TABLE users ADD COLUMN last_rotate_time INTEGER DEFAULT 0",
        )
        .run();
    } catch (e) {}
    try {
      await db
        .prepare(
          "ALTER TABLE users ADD COLUMN auto_rotate_user_proxy INTEGER DEFAULT 0",
        )
        .run();
    } catch (e) {}
    schemaEnsured = true;
...
const SubscriptionService = {
  async generateText(user, host) {
    ...
    const limitGB = user.limit_gb || 0;
    const usedReq = user.used_req || 0;
    const limitReq = user.limit_req || 0;
    const ipLimit = user.ip_limit || user.max_connections || "∞";
    ...
    if (limitGB > 0) {
      const rem = limitGB - usedGB;
      remainingGB = rem > 0 ? rem.toFixed(2) + "GB" : "0GB";
      totalGB = limitGB.toFixed(2) + "GB";
      if (usedGB >= limitGB) statusIcon = "⚠️";
    }

    if (limitReq > 0) {
      const rem = limitReq - usedReq;
      remainingReq = rem > 0 ? rem.toLocaleString() : "0";
      totalReq = limitReq.toLocaleString();
      if (usedReq >= limitReq) statusIcon = "⚠️";
    }

    if (user.expiry_days && user.created_at) {
      const created = new Date(user.created_at);
      const expiryDate = new Date(
        created.getTime() + user.expiry_days * 24 * 60 * 60 * 1000,
      );
      const diffDays = Math.ceil(
        (expiryDate.getTime() - Date.now()) / (1000 * 60 * 60 * 24),
      );
      expiryInfo = diffDays > 0 ? diffDays + " Days" : "Expired";
      if (diffDays <= 0) statusIcon = "⛔";
    }

    if (user.is_active === 0) statusIcon = "⛔";

    const userInfo = `${statusIcon} ${user.username} | 📦 ${usedGB.toFixed(2)}GB/${totalGB} | 📈 ${usedReq.toLocaleString()}/${totalReq} | 👥 ${onlineCount}/${ipLimit} | ⏳ ${expiryInfo}`;
      ...
      try {
        ...
        if (res.ok) {
          ...
          const found = locations.find(
            (l) =>
              l.iata &&
              l.iata.toUpperCase() === user.user_proxy_iata.toUpperCase(),
          );
          if (found && found.cca2) {
    ...
    ips.forEach((ip) => {
      ports.forEach((portStr) => {
        ...
        const userFrag =
          user.frag_len && user.frag_int
            ? "&fragment=" + user.frag_len + "," + user.frag_int
            : "";

        const remark = `${flagEmoji} ${user.username} | `;
    ...
    const subHeader = [
      "# ========================================",
      "# 📡 PX Panel - Subscription",
      "# 👤 User: " + user.username,
      "# 📦 Volume: " +
        (limitGB > 0
          ? usedGB.toFixed(2) + "GB / " + limitGB.toFixed(2) + "GB"
          : "Unlimited"),
      "# 📈 Requests: " +
        (limitReq > 0
          ? usedReq.toLocaleString() + " / " + limitReq.toLocaleString()
          : "Unlimited"),
      "# 👥 Max Users: " + ipLimit,
      "# 👤 Online: " + onlineCount,
      "# ⏳ Expiry: " + expiryInfo,
    ...
    const plainContent = subHeader + links.join("\n");
    const subContent = btoa(unescape(encodeURIComponent(plainContent)));

    const downloadBytes = Math.floor(usedGB * 1073741824);
    const totalBytes = limitGB > 0 ? Math.floor(limitGB * 1073741824) : 0;
    let expireTimestamp = 0;
    if (user.expiry_days && user.created_at) {
      expireTimestamp = Math.floor(
        (new Date(user.created_at).getTime() + user.expiry_days * 86400000) /
          1000,
      );
    }
...
//       "%E2%9A%A0%EF%B8%8F%D9%BE%D9%86%D9%84%20%D8%B1%D8%A7%DB%8C%DA%AF%D8%A7%D9%86%20%D9%88%20%D8%BA%DB%8C%D8%B1%20%D9%82%D8%A7%D8%A8%D9%84%20%D9%81%D8%B1%D9%88%D8%B4%E2%9A%A0%EF%B8%8F",
//     );
//     const m2 = decodeURIComponent(
//       "%F0%9F%9A%80%40PANEL_ZEUS%20%D8%B3%D8%A7%D8%AE%D8%AA%20%D8%B1%D8%A7%DB%8C%DA%AF%D8%A7%D9%86%F0%9F%9A%80",
//     );
//     links.push(
//       atob("dmxlc3M6Ly8=") +
//         user.uuid +
...
//         "@" +
//         host +
//         ":80?path=%2FPANEL_ZEUS&security=none&encryption=none&host=" +
//         host +
//         "&fp=" +
//         fp +
//         "&type=ws#" +
//         encodeURIComponent(infoRemark),
//     );
//     let countryCode = "";
//     if (user.user_proxy_iata) {
//       try {
...
//     const totalBytes = user.limit_gb
//       ? Math.floor(user.limit_gb * 1073741824)
//       : 0;
//     let expireTimestamp = 0;
//     if (user.expiry_days && user.created_at) {
//       expireTimestamp = Math.floor(
    ...
    if (activeCount <= 0 || now - lastActive > 20000) {
      ...
      try {
        await env.DB.prepare(
          ...
          .bind(deltaGb, deltaGb, cachedReqs, uname)
          .run();
      } catch (e) {
...
async function handlevIees(env, storedData = null, ctx = null, request = null) {
  ...
  function addBytes(bytes) {
    ...
    if (
      ...
      (current > 0 && now - lastDbWrite > 60000)
    ) {
      ...
      let writeTask = async () => {
        try {
          await env.DB.prepare(
            ...
            .bind(deltaGb, deltaGb, toCommitReq, username)
            .run();
        } catch (e) {
          console.error(e.message);
        } finally {
  ...
  const setOffline = () => {
    ...
    if (clientIP && clientIP !== "unknown" && validUUID) {
      const removeIpTask = async () => {
        try {
          const user = await env.DB.prepare(
            "SELECT active_ips FROM users WHERE uuid = ?",
          )
            .bind(validUUID)
            .first();
          if (user) {
            console.log(
              `[setOffline Task] DB active_ips for ${uname}: ${user.active_ips}`,
            );
            let activeIps = JSON.parse(user.active_ips || "{}");
            if (activeIps[clientIP]) {
              if (typeof activeIps[clientIP] === "object") {
                activeIps[clientIP].count =
                  (activeIps[clientIP].count || 1) - 1;
              ...
              await env.DB.prepare(
                "UPDATE users SET active_ips = ? WHERE uuid = ?",
              )
                .bind(JSON.stringify(activeIps), validUUID)
                .run();
              console.log(
                `[setOffline Task] Updated active_ips in DB to: ${JSON.stringify(activeIps)}`,
              );
            } else {
              console.log(
                `[setOffline Task] IP ${clientIP} not found in user's active_ips`,
              );
            }
          }
        } catch (e) {
    ...
    if (activeCount <= 0) {
      ...
      if (
        (cachedBytes > 0 || cachedReqs > 0) &&
        !GLOBAL_WRITE_LOCK.get(uname)
      ) {
        GLOBAL_WRITE_LOCK.set(uname, true);
      ...
      try {
        ...
        if (!validUUID) return;
        {
          ...
          if (!user || user.is_active === 0) {
            ...
            if (
              user.limit_req &&
              user.used_req + (USER_REQ_CACHE.get(username) || 0) >=
                user.limit_req
            ) {
              isExpired = true;
            }
            ...
            if (!isExpired && clientIP && clientIP !== "unknown") {
              ...
              if (!activeIps[clientIP]) {
                ...
                console.log(
                  `[Heartbeat] IP ${clientIP} expired from active_ips due to inactivity.`,
                );
              } else {
                const sortedIps = Object.keys(activeIps).sort((a, b) => {
                  const tA =
                    activeIps[a] && typeof activeIps[a] === "object"
                      ? activeIps[a].timestamp
                      : activeIps[a];
                  const tB =
                    activeIps[b] && typeof activeIps[b] === "object"
                      ? activeIps[b].timestamp
                      : activeIps[b];
                  return tB - tA;
                });
                ...
                if (
                  user.ip_limit &&
                  user.ip_limit > 0 &&
                  clientIpIndex >= user.ip_limit
                ) {
                  isIpLimitExpired = true;
                  console.log(
                    `[Heartbeat] IP Limit Exceeded. Client IP index ${clientIpIndex} >= limit ${user.ip_limit}.`,
                  );
                }
              }
          ...
          if (isExpired) {
            await env.DB.prepare(
              "UPDATE users SET is_active = 0, last_active = 0 WHERE uuid = ?",
            )
              .bind(validUUID)
              .run();
          ...
          if (now - lastRecorded > 25000 || updatedActiveIps !== null) {
            ...
            if (updatedActiveIps !== null) {
              await env.DB.prepare(
                "UPDATE users SET last_active = ?, active_ips = ? WHERE username = ?",
              )
                .bind(now, updatedActiveIps, username)
                .run();
            } else {
              await env.DB.prepare(
  ...
  const processWsMessage = async (chunk) => {
    ...
    if (!isHeaderParsed) {
      ...
      let user = null;
      try {
        user = await env.DB.prepare("SELECT * FROM users WHERE uuid = ?")
          .bind(reqUUID)
          .first();
      } catch (e) {}
      if (!user) {
      ...
      if (
        user.limit_req &&
        user.used_req + (USER_REQ_CACHE.get(username) || 0) > user.limit_req
      ) {
        serverSock.close();
        return;
      }
      ...
      if (clientIP && clientIP !== "unknown") {
        console.log(
          `[vIees Handshake] User: ${user.username}, clientIP: ${clientIP}, active_ips in DB: ${user.active_ips}`,
        );
        let activeIps = {};
        try {
          activeIps = JSON.parse(user.active_ips || "{}");
        ...
        if (!activeIps[clientIP]) {
          ...
          console.log(
            `[vIees Handshake] Non-expired active IPs: ${JSON.stringify(activeIps)}, count: ${sortedIps.length}, limit: ${user.ip_limit}`,
          );
          if (
            user.ip_limit &&
            user.ip_limit > 0 &&
            sortedIps.length >= user.ip_limit
          ) {
            console.log(
              `[vIees Handshake] BLOCKED user ${user.username} because sortedIps.length (${sortedIps.length}) >= limit (${user.ip_limit})`,
            );
            serverSock.close();
            return;
          }
          ...
          console.log(
            `[vIees Handshake] Reconnected from same IP: ${clientIP}, count: ${activeIps[clientIP].count}`,
          );
        }
        try {
          await env.DB.prepare(
            "UPDATE users SET active_ips = ?, last_active = ? WHERE uuid = ?",
          )
            .bind(JSON.stringify(activeIps), now, reqUUID)
            .run();
          console.log(
            `[vIees Handshake] Successfully updated active_ips to: ${JSON.stringify(activeIps)}`,
          );
        } catch (e) {
      ...
      try {
        ...
        if (addrType === 1) {
          ...
          addr = new TextDecoder().decode(
            chunkBuffer.slice(offset, offset + domainLen),
          );
          offset += domainLen;
        } else if (addrType === 3) {
          const v6 = [];
          for (let i = 0; i < 8; i++) {
            v6.push(
              ((chunkBuffer[offset++] << 8) | chunkBuffer[offset++]).toString(
                16,
              ),
            );
          }
        ...
        if (
          (user.block_ads === 1 || user.block_porn === 1) &&
          addrType === 2 &&
          port !== 53
        ) {
          try {
            const dnsCheck = await dohQuery(addr, "A", targetDoh);
            const isBlocked = dnsCheck.some(
              (r) =>
                r.data === "0.0.0.0" ||
                r.data === "::" ||
                r.data === "176.103.130.130",
            );
            if (isBlocked) {
              serverSock.close();
              return;
            }
            const validIps = dnsCheck.filter(
              (r) =>
                r.type === 1 && typeof r.data === "string" && isIPv4(r.data),
            );
        ...
        const connectTCP = async (dataPayload = null, useFallback = true) => {
          ...
          const task = (async () => {
            ...
            if (socks5) {
              try {
                ...
                if (user.auto_rotate_user_proxy === 1) {
                  const replaceTask = replaceBrokenProxy(
                    user.username,
                    env,
                    socks5,
                  );
              ...
              if (user?.user_proxy_iata) {
                activeProxyIP =
                  user.user_proxy_iata.toLowerCase() + ".proxyip.cmliussss.net";
              ...
              if (activeProxyIP) {
                if (activeProxyIP.startsWith("[")) {
                  ...
                  if (closeIdx !== -1) {
                    ...
                    if (
                      activeProxyIP.length > closeIdx + 1 &&
                      activeProxyIP[closeIdx + 1] === ":"
                    ) {
                      fPort =
                        parseInt(activeProxyIP.substring(closeIdx + 2)) || port;
                    }
                  }
                } else {
                  ...
                  if (
                    lastColon !== -1 &&
                    activeProxyIP.indexOf(":") === lastColon
                  ) {
                    fHost = activeProxyIP.substring(0, lastColon);
                    fPort =
                      parseInt(activeProxyIP.substring(lastColon + 1)) || port;
                  } else {
                    fHost = activeProxyIP;
                  }
                }
              ...
              const isCustomProxy =
                activeProxyIP && activeProxyIP !== "proxyip.cmliussss.net";
              if (isCustomProxy) {
                try {
            ...
            s.closed
              .catch(() => {})
              .finally(() => closeSocketQuietly(serverSock));
...
async function connectDirect(
  ...
  port,
  initialData = null,
  targetDoh = "https://cloudflare-dns.com/dns-query",
) {
...
async function forwardvIeesUDP(
  udpChunk,
  webSocket,
  respHeader,
  onBytes,
        ...
        async write(chunk) {
          ...
          if (vIeesHeader) {
            const merged = new Uint8Array(
              vIeesHeader.length + response.byteLength,
            );
...
function trackRequest(env, ctx) {
  ...
  const now = Date.now();
  if (
    (now - GLOBAL_LAST_REQ_WRITE > 900000 || GLOBAL_REQ_COUNT > 5000) &&
    GLOBAL_REQ_COUNT > 0
  ) {
...
async function connectProxy(proxyStr, destAddr, destPort, initialData) {
  ...
  let cleanStr = normalized.replace(
    /^(socks4|socks5|socks|http|https):\/\//i,
    "",
  );
  if (isHttp) {
  ...
  if (remain.startsWith("[")) {
    ...
    if (closeIdx !== -1) {
      ...
      if (remain.length > closeIdx + 1 && remain[closeIdx + 1] === ":")
        port = parseInt(remain.substring(closeIdx + 2)) || defaultPort;
    }
  } else {
    const lastColon = remain.lastIndexOf(":");
    if (lastColon !== -1 && remain.indexOf(":") === lastColon) {
...
<body class="bg-gray-50 text-gray-900 dark:bg-amoled-bg dark:text-zinc-100 min-h-screen flex items-center justify-center p-4">
    <div class="w-full max-w-md bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-md shadow-xl p-8 text-center flex flex-col items-center gap-4">
        ...
        <h2 class="text-xl font-bold text-gray-900 dark:text-white">ارور 404</h2>
        <p class="text-sm text-gray-600 dark:text-gray-400 leading-relaxed mt-2">
           ادرس مورد نظر یافت نشد
            <span class="inline-block px-2 py-1 bg-gray-100 dark:bg-amoled-input border border-gray-200 dark:border-zinc-800 rounded-md font-mono text-blue-500 font-bold mx-1 shadow-sm" dir="ltr">ادرس اشتباه وارد شده و یا حذف شده</span> 
        </p>
        <button onclick="window.location.href='https://telegram.me/pxconfir'" 
        class="mt-4 w-full py-2.5 bg-gradient-to-b from-white/90 via-blue-50/80 to-blue-100/70 dark:from-blue-950/40 dark:via-blue-900/30 dark:to-blue-800/40 border border-blue-200 dark:border-blue-800/50 hover:from-white hover:via-blue-50 hover:to-blue-200/80 dark:hover:from-blue-900/60 dark:hover:via-blue-800/50 dark:hover:to-blue-700/50 transition-all duration-200 text-blue-600 dark:text-blue-300 font-bold rounded-md text-sm shadow-sm hover:shadow flex items-center justify-center gap-2">
...
<body class="bg-gray-50 text-gray-900 dark:bg-amoled-bg dark:text-zinc-100 min-h-screen flex items-center justify-center p-4">
    <div class="w-full max-w-md bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-md shadow-xl p-6">
        ...
        <p class="text-sm text-gray-500 dark:text-gray-400 text-center mb-6">چون اولین باره - یه رمز عبور قوی انتخاب کن</p>
    ...
    <div class="w-full max-w-md bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-md shadow-xl p-6">
        ...
        <div id="recovery-section" class="hidden">
            ...
            <div class="mb-5 p-3 bg-orange-50 dark:bg-orange-900/20 border border-orange-200 dark:border-orange-800/50 rounded-md text-xs leading-relaxed text-orange-800 dark:text-orange-300">
                ...
                <a href="https://dash.cloudflare.com/profile/api-tokens?permissionGroupKeys=%5B%7B%22key%22%3A%22workers_scripts%22%2C%22type%22%3A%22edit%22%7D%2C%7B%22key%22%3A%22workers_kv_storage%22%2C%22type%22%3A%22edit%22%7D%2C%7B%22key%22%3A%22d1%22%2C%22type%22%3A%22edit%22%7D%2C%7B%22key%22%3A%22account_settings%22%2C%22type%22%3A%22read%22%7D%2C%7B%22key%22%3A%22workers_subdomain%22%2C%22type%22%3A%22edit%22%7D%2C%7B%22key%22%3A%22account_analytics%22%2C%22type%22%3A%22read%22%7D%5D&accountId=*&zoneId=all&name=PX-Deployer-Token" target="_blank" class="mt-3 w-full flex items-center justify-center gap-2 py-2 bg-transparent border-2 border-green-600 text-green-700 hover:bg-green-900/20 hover:text-green-800 dark:border-green-500 dark:text-green-500 dark:hover:bg-green-900/40 dark:hover:text-green-400 rounded-md font-bold transition shadow-md">
        ...
        <div class="max-w-6xl mx-auto flex flex-col md:flex-row justify-between items-center gap-4">
            <div class="flex flex-row flex-wrap justify-center items-center gap-3 w-full md:w-auto">
                <h1 class="text-lg font-bold flex items-center gap-2" dir="ltr">
                    P X
                    <span id="panel-version" class="text-xs px-2 py-0.5 font-semibold bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-400 rounded-full">NEW UPDATE</span>
                </h1>
...
<button onclick="openCreateModal()" 
        class="p-2 rounded-md 
               bg-gradient-to-b from-white/90 via-blue-50/80 to-blue-100/70 
               dark:from-blue-950/40 dark:via-blue-900/30 dark:to-blue-800/40
               border border-blue-200 dark:border-blue-800/50 
               hover:from-white hover:via-blue-50 hover:to-blue-200/80
               dark:hover:from-blue-900/60 dark:hover:via-blue-800/50 dark:hover:to-blue-700/50
               transition-all duration-200 
               text-blue-600 dark:text-blue-300 shadow-sm hover:shadow" 
        title="ایجاد کانفیگ">
    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<button id="theme-toggle" 
        class="p-2 rounded-md 
               bg-gradient-to-b from-white/90 via-amber-50/80 to-amber-100/70 
               dark:from-amber-950/40 dark:via-amber-900/30 dark:to-amber-800/40
               border border-amber-200 dark:border-amber-800/50 
               hover:from-white hover:via-amber-50 hover:to-amber-200/80
               dark:hover:from-amber-900/60 dark:hover:via-amber-800/50 dark:hover:to-amber-700/50
               transition-all duration-200 
               text-amber-600 dark:text-amber-300 shadow-sm hover:shadow"
        title="تغییر تم">
...
<button onclick="toggleSettingsModal(true)" 
        class="p-2 rounded-md 
               bg-gradient-to-b from-white/90 via-gray-50/80 to-gray-100/70 
               dark:from-gray-800/40 dark:via-gray-700/30 dark:to-gray-600/40
               border border-gray-200 dark:border-gray-700/50 
               hover:from-white hover:via-gray-50 hover:to-gray-200/80
               dark:hover:from-gray-700/60 dark:hover:via-gray-600/50 dark:hover:to-gray-500/50
               transition-all duration-200 
               text-gray-600 dark:text-gray-300 shadow-sm hover:shadow" 
        title="تنظیمات">
    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<button onclick="logoutAdmin()" 
        class="p-2 rounded-md 
               bg-gradient-to-b from-white/90 via-red-50/80 to-red-100/70 
               dark:from-red-950/40 dark:via-red-900/30 dark:to-red-800/40
               border border-red-200 dark:border-red-800/50 
               hover:from-white hover:via-red-50 hover:to-red-200/80
               dark:hover:from-red-900/60 dark:hover:via-red-800/50 dark:hover:to-red-700/50
               transition-all duration-200 
               text-red-600 dark:text-red-300 shadow-sm hover:shadow" 
        title="خروج">
    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<div class="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
    <div class="bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-md p-2.5 shadow-sm flex flex-col justify-center gap-1 hover:shadow-md hover:border-indigo-400 dark:hover:border-indigo-500/50 transition duration-300 relative overflow-hidden group min-h-[64px]">
        ...
        <div class="flex items-center justify-between relative z-10">
            <span class="text-[11px] sm:text-xs font-semibold text-gray-500 dark:text-zinc-400 whitespace-nowrap">تعداد کل کانفیگ ها</span>
    ...
    <div class="bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-md p-2.5 shadow-sm flex flex-col justify-center gap-1 hover:shadow-md hover:border-emerald-400 dark:hover:border-emerald-500/50 transition duration-300 relative overflow-hidden group min-h-[64px]">
        ...
        <div class="flex items-center justify-between relative z-10">
            <span class="text-[11px] sm:text-xs font-semibold text-gray-500 dark:text-zinc-400 whitespace-nowrap">کانفیگ فعال (درحال کار)</span>
        ...
        <div class="flex items-end justify-between relative z-10 w-full mt-0.5">
            ...
            <span class="text-[9px] text-emerald-500 dark:text-emerald-400 flex items-center gap-1 font-medium whitespace-nowrap leading-none mb-0.5">
               ...
               کل کانفیگ های درحال کار و متصل
            </span>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-md p-2.5 shadow-sm flex flex-col justify-center gap-1 hover:shadow-md hover:border-blue-400 dark:hover:border-blue-500/50 transition duration-300 relative overflow-hidden group min-h-[64px]">
        ...
        <div class="flex items-center justify-between relative z-10">
            <span class="text-[11px] sm:text-xs font-semibold text-gray-500 dark:text-zinc-400 whitespace-nowrap">ترافیک مصرفی</span>
        ...
        <div id="loading-state" class="text-center py-12">
            <span class="text-gray-500 dark:text-gray-400">در حال لود کردن...</span>
        </div>
    ...
    <div class="flex items-center gap-2 w-full md:w-auto">
        <div class="relative flex-1 min-w-0">
            <select id="filter-status" onchange="filterAndRenderUsers()" class="w-full appearance-none px-2 py-1.5 pl-7 bg-gray-50 dark:bg-amoled-input border border-gray-300 dark:border-amoled-border rounded-md text-xs focus:outline-none focus:ring-2 focus:ring-blue-500 text-gray-700 dark:text-zinc-300 cursor-pointer truncate">
            ...
            <div class="absolute inset-y-0 left-0 flex items-center pl-1.5 pointer-events-none text-gray-500 dark:text-gray-400">
        ...
        <div class="relative flex-1 min-w-0">
            <select id="sort-users" onchange="filterAndRenderUsers()" class="w-full appearance-none px-2 py-1.5 pl-7 bg-gray-50 dark:bg-amoled-input border border-gray-300 dark:border-amoled-border rounded-md text-xs focus:outline-none focus:ring-2 focus:ring-blue-500 text-gray-700 dark:text-zinc-300 cursor-pointer truncate">
    ...
    <div class="flex items-center gap-2">
        <button onclick="refreshUsersList()" 
                class="p-2 rounded-md bg-blue-50 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-800/50 hover:bg-blue-100 dark:hover:bg-blue-900/50 hover:border-blue-300 dark:hover:border-blue-700 transition-all duration-200 text-blue-600 dark:text-blue-400 hover:scale-110 active:scale-95 shadow-sm hover:shadow-md group" 
                title="رفرش لیست کاربران">
            <svg class="w-5 h-5 group-hover:animate-spin transition-all duration-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <table class="w-full text-right border-collapse">
        <thead>
            <tr class="bg-gray-100/80 dark:bg-zinc-900/50 border-b border-gray-200 dark:border-amoled-border text-xs text-gray-500 dark:text-gray-400 text-center">
...
<div id="empty-state" class="hidden p-10 border-2 border-dashed border-blue-300/60 dark:border-blue-700/50 bg-blue-50/50 dark:bg-blue-950/20 rounded-md text-center">
    <div class="flex flex-col items-center gap-3">
        <div class="inline-flex items-center justify-center w-16 h-16 rounded-full bg-blue-100/80 dark:bg-blue-900/30 text-blue-500 dark:text-blue-400">
            <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.172 16.172a4 4 0 015.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>
            </svg>
        </div>
        <p class="text-blue-600 dark:text-blue-400 font-bold text-lg">کانفیگی پیدا نشد</p>
        <p class="text-sm text-gray-500 dark:text-gray-400">برای شروع، اولین کانفیگ را خودت بساز</p>
        <button onclick="openCreateModal()" class="mt-2 px-6 py-2.5 bg-blue-600 hover:bg-blue-700 active:scale-95 transition-all duration-200 text-white rounded-md font-medium text-sm shadow-md hover:shadow-lg">
            ساخت کانفیگ جدید
        </button>
    </div>
</div>
...
<div id="usage-warning-modal" class="fixed inset-0 z-[80] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm opacity-0 pointer-events-none transition-all duration-300 ease-out">
    <div class="w-full max-w-md bg-white dark:bg-gray-900 border border-orange-500/50 rounded-md shadow-2xl overflow-hidden p-6 text-center transition-all transform duration-300 opacity-0 scale-95 ease-out">
        ...
        <h3 class="font-black text-xl text-gray-900 dark:text-white mb-2">هشدار محدودیت درخواست روزانه کلودفلر</h3>
        ...
        <button onclick="closeUsageWarning()" class="w-full py-3.5 bg-green-600 hover:bg-green-700 active:scale-95 transition-all duration-200 text-white font-black rounded-md text-sm shadow-lg hover:shadow-xl">
            متوجه شدم
        </button>
    </div>
</div>
...
<div id="free-panel-warning-modal" class="fixed inset-0 z-[85] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm opacity-0 pointer-events-none transition-all duration-300 ease-out">
    <div class="w-full max-w-md bg-white dark:bg-amoled-card border border-rose-500/50 rounded-md shadow-2xl overflow-hidden p-6 text-center transition-all transform duration-300 opacity-0 scale-95 ease-out">
       <div class="inline-flex items-center justify-center w-16 h-16 rounded-full bg-rose-950/20 dark:bg-rose-950/40 text-rose-800 dark:text-rose-500 mb-4 shadow-inner border border-rose-900/20">
    <svg class="w-8 h-8" fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24">
        <path stroke-linecap="round" stroke-linejoin="round" d="M12 2s-2 3.5-2 5.5s2 4.5 2 4.5s2-2.5 2-4.5S12 2 12 2Z" />
        <path stroke-linecap="round" stroke-linejoin="round" d="M9 12h6v9H9z" />
    </svg>
</div>
        <h3 class="font-black text-xl text-gray-900 dark:text-white mb-2">دی ماه - 1404</h3>
        <p class="text-sm text-gray-600 dark:text-gray-400 mb-6 leading-relaxed font-medium">
           🖤 به یاد تمام عزیزانی که در دی‌ماه ۱۴۰۴ جان خود را در ایران از دست دادند
گاهی بعضی از روزهای تقویم، فقط یک تاریخ نیستند؛ یادآور لحظه‌هایی هستند که دل هزاران خانواده برای همیشه داغدار شد. دی‌ماه ۱۴۰۴ برای بسیاری از مردم ایران با اندوه، اشک و خاطراتی گره خورد که هیچ‌گاه از ذهنشان پاک نخواهد شد.
امروز با نهایت احترام، یاد تمام کسانی را گرامی می‌داریم که در آن روزها جان خود را از دست دادند. 🕊 فارغ از هر نام، جایگاه، باور یا دیدگاه، از دست رفتن هر انسان، زخمی بر قلب خانواده، دوستان و جامعه است.
🕯 برای پدرانی که دیگر به خانه بازنگشتند، برای مادرانی که آغوششان خالی ماند، برای جوانانی که آرزوهایشان نیمه‌تمام ماند، برای کودکانی که فرصت بزرگ شدن را از دست دادند و برای تمام انسان‌هایی که زندگی‌شان در آن روزها پایان یافت، با احترام سر تعظیم فرود می‌آوریم.
هیچ واژه‌ای نمی‌تواند درد خانواده‌هایی را که عزیزی را از دست داده‌اند توصیف کند. تنها می‌توان امیدوار بود که صبر، همدلی و مهربانی مردم، اندکی از سنگینی این غم را کمتر کند. 🤍
یاد و خاطره کسانی که از میان ما رفتند، همواره در قلب عزیزانشان زنده خواهد ماند. امیدواریم روزی برسد که هیچ خانواده‌ای داغدار از دست دادن عزیزش نباشد و آرامش، امنیت و امید، سهم همه مردم این سرزمین باشد. 🌿
🖤 یادشان گرامی، نامشان ماندگار و خاطره‌شان جاودان.
        </p>
<button onclick="closeFreePanelWarning()" class="w-full py-3 bg-transparent border border-zinc-400 text-zinc-800 hover:bg-zinc-900 hover:text-white dark:border-zinc-700 dark:text-zinc-300 dark:hover:bg-zinc-100 dark:hover:text-zinc-900 font-medium rounded-lg text-sm transition duration-300">
  روحشون شاد🤍
</button>
        ...
        <div id="user-modal-card" class="w-full max-w-xl lg:max-w-[1200px] bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-md shadow-xl overflow-hidden transition-[opacity,transform] duration-200 opacity-0 scale-95 ease-out flex flex-col max-h-[90vh] transform-gpu" style="will-change: transform, opacity;">
            <div class="px-6 py-4 border-b border-gray-150 dark:border-amoled-border flex justify-between items-center bg-gray-50/50 dark:bg-amoled-bg">
                <div class="flex items-center gap-2">
                    ...
                    <h3 id="modal-title" class="font-bold text-gray-900 dark:text-zinc-100 text-base">ایجاد کانفیگ جدید</h3>
                </div>
                        ...
                        <div class="space-y-2.5">
                            <div>
                                ...
                                <div class="relative">
                                    ...
                                    <input type="text" id="input-name" oninput="this.value = this.value.replace(/[^a-zA-Z0-9_-]/g, '')" placeholder="PX_PANEL" maxlength="32" class="w-full pl-3 pr-9 py-1.5 bg-gray-50 dark:bg-amoled-input border border-gray-200 dark:border-amoled-border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500/50 text-xs font-semibold text-gray-800 dark:text-zinc-100 placeholder-gray-400/80 transition" required>
                                </div>
                            </div>
                            <div class="grid grid-cols-2 gap-2.5">
                                ...
                                <div>
                                    <label class="block text-[10px] font-bold text-gray-500 dark:text-zinc-400 mb-1 uppercase tracking-wider">محدودیت تعداد همزمان متصل</label>
                                    <div class="relative">
                                        <span class="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none text-gray-400">
                    ...
                    <div class="flex flex-col gap-2 pt-4 lg:pt-0 border-t-2 lg:border-t-0 border-gray-300 dark:border-amoled-border justify-between">
                        <div class="flex flex-col flex-1">
                            ...
                            <div class="grid grid-cols-2 gap-2 mb-2 w-full">
                                ...
                                <a href="https://github.com/IR-NETLIFY/zeus-relay" target="_blank" class="text-[11px] bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 px-2 py-2 rounded border border-blue-200 dark:border-blue-800 hover:bg-blue-100 dark:hover:bg-blue-900/50 transition font-black shadow-sm text-center whitespace-nowrap">ساخت پروکسی شخصی</a>
                            </div>
...
<div id="settings-modal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm opacity-0 pointer-events-none transition-all duration-300 ease-out">
    <div class="w-full max-w-md bg-gradient-to-b from-white/95 via-blue-50/90 to-blue-100/80 dark:from-gray-900/95 dark:via-blue-950/40 dark:to-blue-900/30 border border-blue-200 dark:border-blue-800/50 rounded-md shadow-xl overflow-hidden transition-all transform duration-300 opacity-0 scale-95 ease-out flex flex-col max-h-[90vh]">
        <div class="px-6 py-4 border-b border-blue-200 dark:border-blue-800/50 flex justify-between items-center bg-gradient-to-r from-blue-50/80 to-blue-100/60 dark:from-blue-950/30 dark:to-blue-900/20">
            ...
            <button onclick="toggleSettingsModal(false)" class="p-1.5 rounded-md bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-900/50 text-red-600 dark:text-red-500 hover:bg-red-100 dark:hover:bg-red-900/50 transition-all duration-200 shadow-sm hover:scale-110 active:scale-95">
        ...
        <div class="p-6 space-y-4 overflow-y-auto flex-1 overscroll-contain">
            <div class="pt-2">
                <label class="block text-sm font-medium mb-1.5 text-blue-800 dark:text-blue-300 flex items-center gap-2">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="relative">
                    <select id="refresh-rate-select" onchange="changeRefreshRate(this.value)" class="w-full pl-8 pr-3 py-2.5 bg-white dark:bg-gray-800 border border-blue-200 dark:border-blue-800/50 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 text-gray-700 dark:text-zinc-200 cursor-pointer appearance-none">
                        <option value="1000">1 ثانیه</option>
                        <option value="2000" selected>۲ ثانیه (پیش‌فرض)</option>
                        <option value="5000">5 ثانیه</option>
                        ...
                        <option value="600000">10 دقیقه</option>
                    </select>
                    <div class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3 text-blue-500 dark:text-blue-400">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/>
                        </svg>
                    </div>
            ...
            <div class="pt-4 border-t-2 border-blue-200 dark:border-blue-800/30">
                <h4 class="text-sm font-bold mb-3 text-blue-800 dark:text-blue-300 flex items-center gap-2">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="space-y-3">
                    <div>
                        <label class="block text-[11px] text-blue-600 dark:text-blue-400 font-medium mb-1">رمز عبور فعلی</label>
                        <input type="password" id="change-pwd-current" class="w-full px-3 py-2 bg-white dark:bg-gray-800 border border-blue-200 dark:border-blue-800/50 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 text-xs font-mono text-center text-gray-900 dark:text-white">
                    </div>
                    <div>
                        <label class="block text-[11px] text-blue-600 dark:text-blue-400 font-medium mb-1">رمز عبور جدید</label>
                        <input type="password" id="change-pwd-new" class="w-full px-3 py-2 bg-white dark:bg-gray-800 border border-blue-200 dark:border-blue-800/50 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 text-xs font-mono text-center text-gray-900 dark:text-white">
                    </div>
                    <button type="button" onclick="changeAdminPassword()" id="change-pwd-btn" class="w-full py-2 bg-gradient-to-b from-white/90 via-blue-50/80 to-blue-100/70 dark:from-blue-950/40 dark:via-blue-900/30 dark:to-blue-800/40 border border-blue-200 dark:border-blue-800/50 hover:from-white hover:via-blue-50 hover:to-blue-200/80 dark:hover:from-blue-900/60 dark:hover:via-blue-800/50 dark:hover:to-blue-700/50 transition-all duration-200 text-blue-600 dark:text-blue-300 font-semibold rounded-md text-xs shadow-sm hover:shadow">
                        تغییر رمز عبور
                    </button>
                </div>
            </div>
            ...
            <div class="pt-4 border-t-2 border-blue-200 dark:border-blue-800/30">
                ...
                <div class="grid grid-cols-2 gap-3">
                    <button type="button" onclick="exportUsersBackup()" class="py-2.5 bg-gradient-to-b from-white/90 via-orange-50/80 to-orange-100/70 dark:from-orange-950/40 dark:via-orange-900/30 dark:to-orange-800/40 border border-orange-200 dark:border-orange-800/50 hover:from-white hover:via-orange-50 hover:to-orange-200/80 dark:hover:from-orange-900/60 dark:hover:via-orange-800/50 dark:hover:to-orange-700/50 transition-all duration-200 text-orange-600 dark:text-orange-300 rounded-md text-xs font-bold flex items-center justify-center gap-1.5 shadow-sm hover:shadow">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/>
                        </svg>
                        پشتیبان گیری
                    </button>
                    <button type="button" onclick="triggerImportBackup()" class="py-2.5 bg-gradient-to-b from-white/90 via-blue-50/80 to-blue-100/70 dark:from-blue-950/40 dark:via-blue-900/30 dark:to-blue-800/40 border border-blue-200 dark:border-blue-800/50 hover:from-white hover:via-blue-50 hover:to-blue-200/80 dark:hover:from-blue-900/60 dark:hover:via-blue-800/50 dark:hover:to-blue-700/50 transition-all duration-200 text-blue-600 dark:text-blue-300 rounded-md text-xs font-bold flex items-center justify-center gap-1.5 shadow-sm hover:shadow">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/>
                        </svg>
                        بازیابی
            ...
            <div class="pt-4 flex gap-3">
                <button type="button" onclick="toggleSettingsModal(false)" class="flex-1 py-2 bg-gradient-to-b from-white/90 via-rose-50/80 to-rose-100/70 dark:from-rose-950/40 dark:via-rose-900/30 dark:to-rose-800/40 border border-rose-200 dark:border-rose-800/50 hover:from-white hover:via-rose-50 hover:to-rose-200/80 dark:hover:from-rose-900/60 dark:hover:via-rose-800/50 dark:hover:to-rose-700/50 transition-all duration-200 text-rose-600 dark:text-rose-300 font-bold rounded-md text-sm shadow-sm hover:shadow">
                    انصراف
                </button>
                <button type="button" onclick="saveSettings()" id="save-settings-btn" class="flex-1 py-2 bg-gradient-to-b from-white/90 via-green-50/80 to-green-100/70 dark:from-green-950/40 dark:via-green-900/30 dark:to-green-800/40 border border-green-200 dark:border-green-800/50 hover:from-white hover:via-green-50 hover:to-green-200/80 dark:hover:from-green-900/60 dark:hover:via-green-800/50 dark:hover:to-green-700/50 transition-all duration-200 text-green-600 dark:text-green-300 font-medium rounded-md text-sm shadow-sm hover:shadow">
                    ذخیره تنظیمات
                </button>
            </div>
        </div>
...
<div id="update-modal" class="fixed inset-0 z-[90] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm opacity-0 pointer-events-none transition-all duration-300 ease-out">
    <div class="w-full max-w-md bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-md shadow-2xl overflow-hidden p-6 text-center transition-all transform duration-300 opacity-0 scale-95 ease-out">
        ...
        <div class="space-y-3">
            ...
            <a href="https://telegram.me/pxconfir" target="_blank" class="w-full py-3.5 bg-orange-50 dark:bg-orange-950/30 hover:bg-orange-100 dark:hover:bg-orange-900/50 text-orange-600 dark:text-orange-500 border border-orange-300 dark:border-orange-500 font-bold rounded-md text-sm transition duration-300 shadow-sm flex items-center justify-center gap-2">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"></path>
                </svg>
                اپدیت دستی (رفتن به تلگرام)
            </a>
        </div>
...
<div id="bulk-actions-bar" class="fixed top-4 left-1/2 -translate-x-1/2 z-[9999] bg-white dark:bg-zinc-900/95 border border-gray-200 dark:border-zinc-800/80 px-6 py-4 rounded-md shadow-2xl flex flex-wrap items-center justify-center gap-4 w-[95%] max-w-4xl transition-all duration-300 transform -translate-y-28 opacity-0 pointer-events-none backdrop-blur-md">
    <div class="flex items-center gap-3">
        <span class="w-3 h-3 bg-blue-500 rounded-full animate-pulse shadow-sm shadow-blue-500/50"></span>
        <span id="bulk-selected-count" class="text-sm font-bold text-gray-800 dark:text-zinc-200">0 کاربر انتخاب شده</span>
    </div>
    <div class="flex flex-wrap gap-2 justify-center">
        <button onclick="bulkToggleStatus(1)" class="px-3 py-1.5 bg-green-50 dark:bg-green-950/20 text-green-700 dark:text-green-500 hover:bg-green-100 dark:hover:bg-green-900/30 rounded-md text-xs font-bold transition border border-green-200 dark:border-green-900/50 flex items-center gap-1.5 hover:scale-105 active:scale-95">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7"/>
            </svg>
            فعال‌سازی
        </button>
        <button onclick="bulkToggleStatus(0)" class="px-3 py-1.5 bg-amber-50 dark:bg-amber-950/20 text-amber-600 dark:text-amber-400 hover:bg-amber-100 dark:hover:bg-amber-900/30 rounded-md text-xs font-bold transition border border-amber-200 dark:border-amber-900/50 flex items-center gap-1.5 hover:scale-105 active:scale-95">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M6 18L18 6M6 6l12 12"/>
            </svg>
            غیرفعال‌سازی
        </button>
        <button onclick="bulkReset('volume')" class="px-3 py-1.5 bg-blue-50 dark:bg-blue-950/20 text-blue-600 dark:text-blue-400 hover:bg-blue-100 dark:hover:bg-blue-900/30 rounded-md text-xs font-bold transition border border-blue-200 dark:border-blue-900/50 flex items-center gap-1.5 hover:scale-105 active:scale-95">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/>
            </svg>
            ریست حجم
        </button>
        <button onclick="bulkReset('req')" class="px-3 py-1.5 bg-sky-50 dark:bg-sky-950/20 text-sky-600 dark:text-sky-400 hover:bg-sky-100 dark:hover:bg-sky-900/30 rounded-md text-xs font-bold transition border border-sky-200 dark:border-sky-900/50 flex items-center gap-1.5 hover:scale-105 active:scale-95">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"/>
            </svg>
            ریست ریکوئست
        </button>
        <button onclick="bulkReset('time')" class="px-3 py-1.5 bg-purple-50 dark:bg-purple-950/20 text-purple-600 dark:text-purple-400 hover:bg-purple-100 dark:hover:bg-purple-900/30 rounded-md text-xs font-bold transition border border-purple-200 dark:border-purple-900/50 flex items-center gap-1.5 hover:scale-105 active:scale-95">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/>
            </svg>
            ریست زمان
        </button>
        <button onclick="bulkDelete()" class="px-3 py-1.5 bg-red-50 dark:bg-red-950/30 text-red-600 dark:text-red-450 hover:bg-red-100 dark:hover:bg-red-900/40 rounded-md text-xs font-bold transition border border-red-200 dark:border-red-900/50 flex items-center gap-1.5 hover:scale-105 active:scale-95">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<footer class="fixed bottom-0 left-0 right-0 border-t border-gray-200 dark:border-amoled-border bg-white dark:bg-amoled-card p-3 shadow-lg z-40">
    <div class="flex flex-col md:flex-row items-center justify-between gap-2 text-sm max-w-7xl mx-auto">
        <div class="text-gray-600 dark:text-gray-400 font-medium">
            <span class="text-blue-600 dark:text-blue-400 font-bold">PX PANEL</span>
            <span class="mx-2 text-gray-300 dark:text-gray-600">|</span>
            <span class="text-gray-500 dark:text-gray-500 text-xs">تمامی حقوق برای PX PANEL محفوظ میباشد </span>
        </div>
        <div class="flex items-center gap-2">
            <span class="text-gray-500 dark:text-gray-500 text-xs">کانال تلگرام:</span>
            <a href="https://telegram.me/pxconfir" 
               target="_blank" 
               rel="noopener noreferrer"
               class="inline-flex items-center gap-1.5 px-3 py-1 rounded-md bg-blue-50 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-800/50 hover:bg-blue-100 dark:hover:bg-blue-900/50 hover:border-blue-300 dark:hover:border-blue-700 transition-all duration-200 text-blue-600 dark:text-blue-400 hover:scale-105 active:scale-95 shadow-sm hover:shadow-md text-xs">
...
async function checkGlobalMessage() {
    return;
		...
		function getvIeesLink(username) {
            ...
            const m2 = decodeURIComponent('%E2%99%A8%EF%B8%8F%20%40PX_PANEL%20%D8%B3%D8%A7%D8%AE%D8%AA%20%D8%B1%D8%A7%DB%8C%DA%AF%D8%A7%D9%86%20%E2%99%A8%EF%B8%8F');
            links.push('vle' + 'ss://' + (user.uuid || '') + '@0.0.0.0:1?encryption=none&security=none&type=ws&host=' + host + '&path=%2FPX_PANEL#' + encodeURIComponent(m1));
            links.push('vle' + 'ss://' + (user.uuid || '') + '@0.0.0.0:1?encryption=none&security=none&type=ws&host=' + host + '&path=%2FPX_PANEL#' + encodeURIComponent(m2));
            let flagEmoji = '🌐';
            if (user.user_proxy_iata) {
                try {
                    const cachedLocations = localStorage.getItem('cached_locations_list');
            ...
            ips.forEach((ip) => {
                ports.forEach((portStr) => {
                    ...
                    links.push('vle' + 'ss://' + (user.uuid || '') + '@' + ip + ':' + portStr + '?path=%2FPX_PANEL&security=' + tlsVal + '&encryption=none&insecure=0&host=' + host + '&fp=' + fp + '&type=ws&allowInsecure=0&sni=' + host + userFrag + '#' + encodeURIComponent(remark));
                });
            });
            return links.join('\\n');
        }
...
function editUser(encodedUsername) {
	...
	toggleModal(true);
        ...
        function renderLocationsUI(locations, activeIata) {
            ...
            let html = '<option value="">🌐 پیش‌فرض  خودکار)</option>';
...
const CURRENT_VERSION = '.NEW UPDATE';
const UPDATE_FIX = "constsCURRENT_VERSION='d.d.d'";
		async function checkForUpdates(isManual = false) {
            try {
                ...
                const res = await fetch('https://raw.githubusercontent.com/iran-px-panel/px/refs/heads/main/px.js?t=' + Date.now());
                ...
                if (latestVersion && latestVersion !== CURRENT_VERSION) {
                    document.getElementById('update-toggle').className = "p-2 rounded-none bg-green-100 dark:bg-green-900/60 border border-green-500 hover:bg-green-200 dark:hover:bg-green-900/80 transition text-green-700 dark:text-green-400 shadow-[0_0_15px_rgba(34,197,94,0.6)] animate-pulse relative";
        ...
        <div class="border-t border-gray-100 dark:border-zinc-800 pt-6 relative z-10">
            ...
            <div class="space-y-3">
                ...
                <button onclick="copyvIeesConfig()" class="w-full flex justify-between items-center px-4 py-3 bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border hover:border-blue-500 dark:hover:border-blue-500 rounded-md text-xs font-medium transition shadow-sm">
                    <span class="flex items-center gap-2">🚀 کپی کـانفـیگ Vless (مستقیم)</span>
        ...
        function getvIeesLink() {
            ...
            ips.forEach(function(ip, ipIndex) {
                ports.forEach(function(portStr) {
                    ...
                    links.push('vle' + 'ss://' + (u.uuid || '') + '@' + ip + ':' + portStr + '?path=%2FPX_PANEL&security=' + tlsVal + '&encryption=none&insecure=0&host=' + host + '&fp=' + fp + '&type=ws&allowInsecure=0&sni=' + host + userFrag + '#' + encodeURIComponent(remark));
                });
            });
            return links.join('\\n');
        }
